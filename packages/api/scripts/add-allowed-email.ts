...
async function main() {
  ...
  if (!email) {
    console.error(
      'Usage: bun run scripts/add-allowed-email.ts <email> --local|--remote',
    )
    process.exit(1)
  }
  ...
  console.log(
    `Adding ${email} to ${isRemote ? 'remote' : 'local'} AllowedEmail...`,
  )

  try {
    await $`bunx wrangler d1 execute ${DB_NAME} ${locationFlag} --command=${sql}`.quiet()
    console.log('Done.')
  } catch (error) {
