...
const Wrapper = styled<WrapperProps>("div")`
  ...
  & {
    ...
    position: relative;
    text-indent: -9999em;
    border-top: ${(p) => p.size / 12}px solid ${(p) => spinnerColors[p.color].bg};
    border-right: ${(p) => p.size / 12}px solid ${(p) => spinnerColors[p.color].bg};
    border-bottom: ${(p) => p.size / 12}px solid ${(p) => spinnerColors[p.color].bg};
    border-left: ${(p) => p.size / 12}px solid ${(p) => spinnerColors[p.color].fg};
